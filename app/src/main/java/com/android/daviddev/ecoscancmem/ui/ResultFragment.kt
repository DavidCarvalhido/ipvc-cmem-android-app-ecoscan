...
class ResultFragment : Fragment() {
    ...
    private fun bindEcopoints(r: ScanResult) {
        ...
        binding.tvEcopointSub.text = "${getString(R.string.ecopoint_accepts)} · ${getString(R.string.ecopoint_nearby)}"
    ...
    private fun bindImpact(r: ScanResult) {
        ...
        binding.tvEnergyValue.text = if (r.energySavedPercent > 0) "${r.energySavedPercent}%" else "—"
    ...
    private fun bindButtons(r: ScanResult) {
        binding.btnSave.setOnClickListener {
            // Guarda na Room DB via ViewModel (implementar na próxima fase)
            Toast.makeText(requireContext(), getString(R.string.saved_ok), Toast.LENGTH_SHORT).show()
    ...
    private fun ecopointInfo(color: EcopointColor) = when (color) {
        EcopointColor.YELLOW -> Triple(getString(R.string.ecopoint_yellow), R.drawable.ic_ecopoint, R.color.teal_bg)
        EcopointColor.BLUE   -> Triple(getString(R.string.ecopoint_blue),   R.drawable.ic_ecopoint, R.color.blue_bg)
        EcopointColor.GREEN  -> Triple(getString(R.string.ecopoint_green),  R.drawable.ic_ecopoint, R.color.green_bg)
        EcopointColor.RED    -> Triple(getString(R.string.ecopoint_red),    R.drawable.ic_ecopoint, R.color.amber_bg)
        EcopointColor.NONE   -> Triple(getString(R.string.ecopoint_none),   R.drawable.ic_ecopoint, R.color.sensor_label)
    }
}